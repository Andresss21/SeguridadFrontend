...
function MisClinicas() {
    ...
    useEffect(() => {
        ...
        const data = axios.get("http://192.168.1.11:8000/api/showApiClinicas/" + id).then((response) => {
            let datos = [];
    ...
    const sucursales = (element)=>{
        axios.defaults.headers.common.Authorization = `Bearer  ${Token}`;
        const datos = axios.get("http://192.168.1.11:8000/api/showApiSucursales/" + element.id).then((response) => {
                let datos = [];
                response.data.map((doc) => {
                    datos.push({
                        id: doc.id,
