...
function Modals(props) {
    ...
    useEffect(() => {
        const data = axios.get("http://192.168.1.11:8000/api/categorias").then((response) => {
            let datos = [];
    ...
    const getDoctores = ((id) => {
        const data = axios.get("http://192.168.1.11:8000/api/showApiDoctores/" + id).then((response) => {
            let datos = [];
            response.data.map((doc) => {
                datos.push({
                    id: doc.id,
